...
public class EfficientDocument extends Document {
	...
	private void processText() {
		...
		for (String token : tokens) {
			...
			if (isWord(token)) {
				++numWords;
				numSyllables += this.countSyllablesFromWord(token);
			} else {
				++numSentences;
			}
		}
	...
	private int countSyllablesFromWord(String word) {
		...
		for (int i = 0; i < cArray.length; i++) {
			if (i == cArray.length - 1 && Character.toLowerCase(cArray[i]) == 'e' && newSyllable
					&& numOfSyllables > 0) {
				numOfSyllables--;
			}
