...
public class DocumentBenchmarking {
	...
	public static void main(String[] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps * increment + start; numToCheck += increment) {
			...
			long startBasicDocument = System.nanoTime();
			BasicDocument basicDocument = new BasicDocument(input);
			basicDocument.getFleschScore();
			long endBasicDocument = System.nanoTime();
			double estTimeBasicDocument = ((double) endBasicDocument - startBasicDocument) / 100000000.0;

			long startEfficient = System.nanoTime();
			EfficientDocument efficientDocument = new EfficientDocument(input);
			efficientDocument.getFleschScore();
			long endEfficient = System.nanoTime();
			double estTimeEfficient = ((double) endEfficient - startEfficient) / 100000000.0;
