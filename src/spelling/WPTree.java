...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) {
		...
		HashSet<String> visited = new HashSet<>(); // to avoid exploring
													// the same
													// string multiple
													// times
		...
		while (!queue.isEmpty() && !visited.contains(word2)) {
			...
			for (String neighbour : neighbours) {
				...
				if (!visited.contains(neighbour)) {
					...
					WPTreeNode childNode = new WPTreeNode(neighbour, curr);
					queue.add(childNode);

					if (word2.equals(neighbour)){
						WPTreeNode resultNode = new WPTreeNode(neighbour, curr);
						return resultNode.buildPathToRoot();
					}
				}
			}
		}
